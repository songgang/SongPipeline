...
% analyze a sub group of metrics according to Eduardo's email
...
idx_COPDg1 = [8,16,19,20,25,37,40];
idx_COPDg2 = [9,10,18,22,26,38,39,52,55,56];
% idx_COPDg2 = [9,10,22,26,38,39,56];
...
COPDg1_list = false(nb_img, 1); COPDg1_list(idx_COPDg1) = 1; COPDg1_list = COPDg1_list & all_done_list;
COPDg2_list = false(nb_img, 1); COPDg2_list(idx_COPDg2) = 1; COPDg2_list = COPDg2_list & all_done_list;
...
idx_PFT_sel = [1,2,3,5,11,13,14];
PFT_data_sel = PFT_data(:, idx_PFT_sel);
PFT_data_label_sel = PFT_data_label(idx_PFT_sel);
...
% noted by GS: Aug 13, 2010, removed, 
%  res-aeroted-Volume and res-moving-aeroted-Volume already excluded
%  airways
...
%% compute the insp-exp / insp 


[idx_COPD', mtable_at_ext(idx_COPD, end) ./ mtable_at_ext(idx_COPD, 1)]


metric_cat_ex = cell(size(mtable_at_ext, 2), 1);
metric_cat_ex{1} = 'TSLVi';
metric_cat_ex{2} = 'TSLVe';
metric_cat_ex{3} = 'EV';

for i=1:nb_thres
     metric_cat_ex{3+i}=['AT at ', sprintf('%d', thres_list(i)), ' HU'];
     metric_cat_ex{3+nb_thres+i}=['EV + AT at ', sprintf('%d', thres_list(i)), ' HU'];
end;
...
[rho_table, pval_table] = get_corr_widx(mtable_at_ext(:, :), PFT_data_sel, COPDg1_list);
display_corr_pattern(rho_table, pval_table, 'Subgroup 1: Air Trapping Volume x PFT ', PFT_data_label_sel, metric_cat_ex, '', 3);

[rho_table, pval_table] = get_corr_widx(mtable_at_ext(:, :), PFT_data_sel, COPDg2_list);
display_corr_pattern(rho_table, pval_table, 'Subgroup 2: Air Trapping Volume x PFT ', PFT_data_label_sel, metric_cat_ex, '', 5);


%% dump out the correlation values

[rho_table, pval_table] = get_corr_widx(mtable_at_ext(:, :), PFT_data_sel, COPDg1_list);
csv_filename = '/mnt/data/PUBLIC/data1/Data/Output/DrewWarrenLungData/ATCases/Dec_06_2008/script/report/dyn4/COPDg1-corr2.csv';
dump_to_csv('correlation', PFT_data_label_sel, metric_cat_ex, rho_table, csv_filename);
csv_filename = '/mnt/data/PUBLIC/data1/Data/Output/DrewWarrenLungData/ATCases/Dec_06_2008/script/report/dyn4/COPDg1-corr2_pval2.csv';
dump_to_csv('p-value', PFT_data_label_sel, metric_cat_ex, pval_table, csv_filename);


[rho_table, pval_table] = get_corr_widx(mtable_at_ext(:, :), PFT_data_sel, COPDg2_list);
csv_filename = '/mnt/data/PUBLIC/data1/Data/Output/DrewWarrenLungData/ATCases/Dec_06_2008/script/report/dyn4/COPDg2-corr2.csv';
dump_to_csv('correlation', PFT_data_label_sel, metric_cat_ex, rho_table, csv_filename);
csv_filename = '/mnt/data/PUBLIC/data1/Data/Output/DrewWarrenLungData/ATCases/Dec_06_2008/script/report/dyn4/COPDg2-corr2_pval2.csv';
dump_to_csv('p-value', PFT_data_label_sel, metric_cat_ex, pval_table, csv_filename);


%% dump out the raw values

csv_filename = '/mnt/data/PUBLIC/data1/Data/Output/DrewWarrenLungData/ATCases/Dec_06_2008/script/report/dyn4/COPDg1-vol.csv';
dump_to_csv('index', metric_cat_ex, find(COPDg1_list), mtable_at_ext(COPDg1_list, :), csv_filename);

csv_filename = '/mnt/data/PUBLIC/data1/Data/Output/DrewWarrenLungData/ATCases/Dec_06_2008/script/report/dyn4/COPDg2-vol.csv';
dump_to_csv('index', metric_cat_ex, find(COPDg2_list), mtable_at_ext(COPDg2_list, :), csv_filename);
...
mtable_at_ext_normal_lung(:, 3:end-1) = (mtable_aero*ones(1,size(mtable_at_ext,2)-3)) - mtable_at_ext_normal_lung(:, 3:end-1);
...
metric_cat_ex{3} = 'Non Emphysema Volume';

for i=1:nb_thres
     metric_cat_ex{3+i}=['Segmented - DAT at ', sprintf('%d', thres_list(i)), ' HU'];
     metric_cat_ex{3+nb_thres+i}=['Normal Lung (at ', sprintf('%d', thres_list(i)), ' HU)'];
end;
metric_cat_ex{end} = 'Calculated FVC';


[rho_table, pval_table] = get_corr_widx(mtable_at_ext_normal_lung(:, :), PFT_data_sel, COPDg1_list);
display_corr_pattern(rho_table, pval_table, 'Subgroup1: Normal Lung Volume x PFT', PFT_data_label_sel, metric_cat_ex, '', 7);


[rho_table, pval_table] = get_corr_widx(mtable_at_ext_normal_lung(:, :), PFT_data_sel, COPDg2_list);
display_corr_pattern(rho_table, pval_table, 'Subgroup2: Normal Lung Volume x PFT', PFT_data_label_sel, metric_cat_ex, '', 9);
...
%% plot the raw values
figure(5); % clf;
plot(transpose(mtable_at_ext(COPDg2_list, :)), '.-');
