...
#   2. emphysema area or non-emphysema with severe air trapping ( threshold < -910 )
MYDO $C3D $OLDRESDIR/fixed_resampled.nii.gz -threshold -Inf -960 1 0 $RESDIR/aeroted_mask_resampled.nii.gz -multiply -o $RESDIR/severe_resampled.nii.gz
...
#   MYDO $C3D $RESDIR/diff_resampled.nii.gz -threshold 0 $T 1 0 $RESDIR/nonsevere_resampled.nii.gz -multiply -o $RESDIR/dynamic"-"$T"_resampled.nii.gz"
#	MYDO $BINDIR/CalculateVolumeFromBinaryImage 3 $RESDIR/dynamic"-"$T"_resampled.nii.gz" > $RESDIR/res-dynamic"-"$T"-Volume.txt" 
#   MYDO rm $RESDIR/dynamic"-"$T"_resampled.nii.gz"
