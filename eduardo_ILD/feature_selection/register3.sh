...
# MYDO $BINDIR/ThresholdImage 3 \
 ...
 MYDO $ANTSDIR/ANTS 3 -o $RESDIR/ants3.nii.gz \
  -i 200x75x25x5 \
  -m PR[$RESDIR/fixed_resampled.nii.gz,$RESDIR/moving_resampled.nii.gz,1,5] \
  -x $RESDIR/mask_resampled.nii.gz \
  -r Gauss[3,0.5] \
  -t SyN[1]
#  --number-of-affine-iterations 0

 MYDO $ANTSDIR/WarpImageMultiTransform 3 $RESDIR/moving_resampled.nii.gz \
   $RESDIR/moving_resampled3_warped.nii.gz \
   -R $RESDIR/fixed_resampled.nii.gz \
   $RESDIR/ants3Warp.nii.gz $RESDIR/ants3Affine.txt
