...
%% plot data points: FEV1/FVC 
figure(3); clf; hold on; plot(ones(1, 14), xdata(1:14, 3), 'b*'); plot(1+ones(1,11), xdata(14+(1:11), 3), 'r*'); hold off; 
%xlabel('  ILD  |  COPD'); 
ylabel('FEV1/FVC');
xlim([0,3]);
set(gca,'XTick',[]);
...
%% plot data points: expiration (G1) lung region volume
figure(5); clf; hold on; plot(ones(1, 14), xdata(1:14, 21+1), 'b*'); plot(1+ones(1,11), xdata(14+(1:11), 21+1), 'r*'); hold off;
xlim([0,3]);
set(gca,'XTick',[]);

% xlabel('Patient'); ylabel('expiration lung volumes');


%% plot data points: expiration (G1) run-length nonuniformity
% figure(5); clf; hold on; plot(1:14, xdata(1:14, 21+24), 'b*'); plot(14+(1:11), xdata(14+(1:11), 21+24), 'r*'); hold off;
figure(5); clf; hold on; plot(ones(1, 14), xdata(1:14, 21+24), 'b*'); plot(1+ones(1,11), xdata(14+(1:11), 21+24), 'r*'); hold off;
xlim([0,3]);
set(gca,'XTick',[]);
% xlabel('Patient'); ylabel('expiration run-length nonuniformity');
%% plot data points: insp-exp lung region volume vs FVC 
figure; hold on;
plot(xdata(2:end, 1), xdata(2:end, 21+31+31+1), '*');
% plot(xdata(2:14, 1), xdata(2:14, 21+31+31+1), '*');
% plot(xdata(15:end, 1), xdata(15:end, 21+31+31+1), '*');
hold off;
xlabel('FVC');
ylabel('insp-exp lung segmentation');
...
% svm_roc = zeros(nb_leave, nb_fea, nb_svm, nb_selection, nb_ROC_points, 2);
 true_positive_rate_MRMR = squeeze(ave_svm_roc(1, 10, 2, 1, :, 1));
 false_positive_rate_MRMR = squeeze(ave_svm_roc(1, 10, 2, 1, :, 2));
%  true_positive_rate = squeeze(svm_roc(1, 1, 1, 2, :, 1));
%  false_positive_rate = squeeze(svm_roc(1, 1, 1, 2, :, 2));

 true_positive_rate_MRMR2 = squeeze(ave_svm_roc(1, 5, 2, 1, :, 1));
 false_positive_rate_MRMR2 = squeeze(ave_svm_roc(1, 5, 2, 1, :, 2));
...
plot(true_positive_rate_MRMR2, false_positive_rate_MRMR2, 'b+-');
plot(true_positive_rate_PFT, false_positive_rate_PFT, 'g.-');
hold off;
xlabel('TPR');
ylabel('FPR');
% legend('with 10 features', 'with FEV1/FVC');
legend('with 10 features', 'with 5 features', 'with FEV1/FVC');
% legend('with FEV1/FVC');
...
% title(sprintf('t=%f,%s', mrmr_threshold_value, svm_opt(1).name));
% title(sprintf('t=%f,%s', mrmr_threshold_value, svm_opt(1).name));
legend('mRMR: PFT', 'mRMR: image', 'mRMR: both');
axis xy;
% axis tight;
axis([1 nb_fea 0 0.15]);
