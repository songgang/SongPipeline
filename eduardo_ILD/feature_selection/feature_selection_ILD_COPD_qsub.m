function [idx_fea_rank, fea_value_list] = feature_selection_ILD_COPD_qsub(mrmr_exec, job_id, xdata, xdata_label, mrmr_labels, selection_method, threshold_value, nb_max_fea)
...
mrmr_input_filename = ['/dev/shm/mrmr_input_tmp-', job_id, '.txt'];
mrmr_output_filename = ['/dev/shm/mrmr_output_tmp-', job_id, '.txt'];
...
command = sprintf('%s -i %s -t %f -m MID -n %d > %s', mrmr_exec, mrmr_input_filename, threshold_value, nb_total+1, mrmr_output_filename);
...
system(sprintf('rm %s', mrmr_input_filename));
system(sprintf('rm %s', mrmr_output_filename));
