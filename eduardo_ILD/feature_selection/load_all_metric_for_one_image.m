function [mvec, out_dim_list, all_done] = load_all_metric_for_one_image(metric_dir, mlist, dim_list)

nb_metric = length(mlist);

mvec = [];
if nargin >= 3 % preallocate space
...
%     end;
    ...
    mymetric = mlist{ii};
    res_filename = ['res-', mymetric, '.txt'];
    fp = fopen([metric_dir, '/', res_filename], 'rt');
%    fp = fopen(fullfile(metric_dir, res_filename), 'rt');
    if fp<0
