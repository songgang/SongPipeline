function [mlist, num_list] = air_trapping_metric_list()
...
for i=1:length(num_list)
    mlist{i} = ['res-dynamic-', sprintf('%d', num_list(i)), '-Median-Volume'];
end;

mlist{length(num_list)+1} = 'res-severe-Volume';
mlist{length(num_list)+2} = 'res-aeroted2-Volume';
mlist{length(num_list)+3} = 'res-moving-aeroted2-Volume';
