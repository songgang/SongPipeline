...
my $test_case = "convert";
my $output_root = "";
my $process_script = "/home/songgang/project/Empire10/Empire10_thesis/Script/SongPipeline/empire10_thesis/convert_mhd_to_nii.sh";
...
for(my $id_task=0; $id_task<@image_tasks; $id_task++) {
	...
	for (my $id_image=0; $id_image< scalar(@current_image_list); $id_image++ ) {		
		...
		print OUTFILE "bash $process_script $image_name\n";
	}
	...
	if ($current_node =~ m/^compute-[01]-\d{1,2}/) { # a valid node example: compute-1-15
		...
		print "DEBUG>> $current_node : no valid node given for qsub\n";
